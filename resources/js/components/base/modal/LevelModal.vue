<template>
  <div class="item-unit-modal">
    <form action="" @submit.prevent="submitLevel">
      <div class="card-body">
        <div class="form-group row">
          <label class="col-sm-4 col-form-label input-label">{{ $t('settings.customization.level.unit_name') }} <span class="required"> *</span></label>
          <div class="col-sm-7">
            <base-input
              ref="name"
...
export default {
  ...
  methods: {
    ...
    async submitLevel () {
      ...
      if (this.isEdit) {
        response = await this.updateLevel(this.formData)
        ...
        try {
          response = await this.addLevel(this.formData)
          if (response.data) {
            this.isLoading = false
